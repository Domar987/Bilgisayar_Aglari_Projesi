...
GORSEL KULLANIMI:
  - Kiz Kulesi gorselini "kiz_kulesi.jpeg" (veya .jpg, .png) olarak
    bu script ile ayni klasore koyun.
  - Desteklenen formatlar: JPEG, JPG, PNG, BMP, WEBP
  - Gorsel bulunamazsa koyu renk degrade arka plan kullanilir.
"""
...
# ─────────────────────────────────────────────────────
#  GORSEL DOSYA AYARI
#  Script ile ayni klasorde aranicak isimler (sirasyla denenecek):
# ─────────────────────────────────────────────────────
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

BG_IMAGE_CANDIDATES = [
    # Orijinal dosya adi varyantlari
    "kız k..jpeg",
    "kız k..jpg",
    "kız k. .jpeg",
    "kız k. .jpg",
    "kiz k..jpeg",
    "kiz k..jpg",
    # Yeniden adlandirilmis varyantlar
    "kiz_kulesi.jpeg",
    "kiz_kulesi.jpg",
    "kiz_kulesi.png",
    "kiz_k_.jpeg",
...
# ── Gorsel arama klasorleri (hepsi denenir) ───────────
ARAMA_KLASORLERI = [
    SCRIPT_DIR,                          # script ile ayni klasor
    os.getcwd(),                         # calisma dizini
    os.path.expanduser("~"),             # kullanici ana dizini
    os.path.expanduser("~/Desktop"),     # masaustu
    os.path.expanduser("~/Downloads"),   # indirmeler
]
...
def gorsel_yolu_bul():
    """
    1. Once bilinen isim listesini dener.
    2. Bulamazsa klasordeki TUM resim dosyalarini tarar,
       adi 'kiz' veya 'k' ile baslayani alir.
    3. O da yoksa klasordeki ilk resim dosyasini alir.
    """
    # 1. Bilinen isimlerle dene
    for klasor in ARAMA_KLASORLERI:
        for isim in BG_IMAGE_CANDIDATES:
            tam_yol = os.path.join(klasor, isim)
            if os.path.isfile(tam_yol):
                print(f"[Metro] Gorsel bulundu: {tam_yol}")
                return tam_yol

    # 2. Klasordeki tum resimleri tara
    for klasor in ARAMA_KLASORLERI:
        ...
        resimler = [
            f for f in dosyalar
            if os.path.splitext(f)[1].lower() in GORSEL_UZANTILARI
        ]
        # Adi 'kiz' veya 'k' ile baslayan varsa onu sec
        oncelikli = [
            f for f in resimler
            if f.lower().startswith(("kiz", "k\u0131z", "k"))
        ]
        secilen = oncelikli[0] if oncelikli else (resimler[0] if resimler else None)
        if secilen:
            tam_yol = os.path.join(klasor, secilen)
            print(f"[Metro] Gorsel tarama ile bulundu: {tam_yol}")
            return tam_yol

    print("[Metro] Hicbir gorsel bulunamadi, varsayilan arka plan kullaniliyor.")
    return None
...
YEREL_KULLANICILAR = {
    ...
    "admin": {"sifre": "admin", "ad_soyad": "Admin Kullanici",  "email": "admin@metro.ist", "bakiye": 500.0, "kart": "0000-0001"},
}

ISTASYONLAR = [
    {"id":"m1s1","isim":"Havalimani",      "hat":"M1","renk":"#c0392b","x":60, "y":510,"lat":40.976,"lon":28.814,"km":0,   "sefer":4,"yog":"Yogun",    "fac":["Asansor","Engelli","Otopark","Bagaj"],"aktarma":[]},
    {"id":"m1s2","isim":"Sefakoy",         "hat":"M1","renk":"#c0392b","x":120,"y":510,"lat":40.998,"lon":28.830,"km":4.2, "sefer":6,"yog":"Orta",     "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m1s3","isim":"Bahcelievler",    "hat":"M1","renk":"#c0392b","x":185,"y":470,"lat":41.002,"lon":28.843,"km":8.1, "sefer":8,"yog":"Az",       "fac":["Asansor"],"aktarma":[]},
    {"id":"m1s4","isim":"Merter",          "hat":"M1","renk":"#c0392b","x":250,"y":430,"lat":41.003,"lon":28.875,"km":11.4,"sefer":5,"yog":"Cok Yogun","fac":["Asansor","Engelli","Bufe"],"aktarma":["M3"]},
    {"id":"m1s5","isim":"Yenibosna",       "hat":"M1","renk":"#c0392b","x":310,"y":390,"lat":41.006,"lon":28.896,"km":14.7,"sefer":3,"yog":"Orta",     "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m1s6","isim":"Yenikapi",        "hat":"M1","renk":"#c0392b","x":365,"y":368,"lat":41.004,"lon":28.951,"km":18.3,"sefer":7,"yog":"Cok Yogun","fac":["Asansor","Engelli","AVM","Bufe"],"aktarma":["M2"]},
    {"id":"m2s1","isim":"Haciosman",       "hat":"M2","renk":"#2471a3","x":80, "y":55, "lat":41.112,"lon":29.030,"km":0,   "sefer":9,"yog":"Az",       "fac":["Asansor","Otopark"],"aktarma":[]},
    {"id":"m2s2","isim":"Darussafaka",     "hat":"M2","renk":"#2471a3","x":80, "y":130,"lat":41.105,"lon":29.026,"km":2.8, "sefer":5,"yog":"Orta",     "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m2s3","isim":"Ataturk Oto.",    "hat":"M2","renk":"#2471a3","x":130,"y":185,"lat":41.097,"lon":29.023,"km":5.2, "sefer":4,"yog":"Orta",     "fac":["Asansor","Engelli","Otopark"],"aktarma":["M3"]},
    {"id":"m2s4","isim":"4.Levent",        "hat":"M2","renk":"#2471a3","x":210,"y":235,"lat":41.082,"lon":29.013,"km":9.0, "sefer":3,"yog":"Yogun",    "fac":["Asansor","Engelli","Bufe"],"aktarma":[]},
    {"id":"m2s5","isim":"Levent",          "hat":"M2","renk":"#2471a3","x":290,"y":272,"lat":41.076,"lon":29.010,"km":11.8,"sefer":6,"yog":"Yogun",    "fac":["Asansor","Engelli","AVM"],"aktarma":[]},
    {"id":"m2s6","isim":"Gayrettepe",      "hat":"M2","renk":"#2471a3","x":365,"y":292,"lat":41.068,"lon":29.001,"km":14.5,"sefer":4,"yog":"Cok Yogun","fac":["Asansor","Engelli","Bufe"],"aktarma":[]},
    {"id":"m2s7","isim":"Sisli",           "hat":"M2","renk":"#2471a3","x":425,"y":292,"lat":41.061,"lon":28.987,"km":17.1,"sefer":5,"yog":"Yogun",    "fac":["Asansor","Engelli","AVM"],"aktarma":[]},
    {"id":"m2s8","isim":"Taksim",          "hat":"M2","renk":"#2471a3","x":488,"y":272,"lat":41.037,"lon":28.985,"km":19.8,"sefer":2,"yog":"Cok Yogun","fac":["Asansor","Engelli","Bufe","Danisma"],"aktarma":[]},
    {"id":"m2s9","isim":"Osmanbey",        "hat":"M2","renk":"#2471a3","x":558,"y":232,"lat":41.047,"lon":28.998,"km":21.3,"sefer":7,"yog":"Orta",     "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m2s10","isim":"Sishane",        "hat":"M2","renk":"#2471a3","x":625,"y":192,"lat":41.028,"lon":28.972,"km":23.0,"sefer":5,"yog":"Orta",     "fac":["Asansor","Engelli","Bufe"],"aktarma":[]},
    {"id":"m2s11","isim":"Halic",          "hat":"M2","renk":"#2471a3","x":685,"y":162,"lat":41.032,"lon":28.952,"km":25.5,"sefer":6,"yog":"Az",       "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m3s1","isim":"Kirazli",         "hat":"M3","renk":"#1e8449","x":130,"y":268,"lat":41.054,"lon":28.800,"km":0,   "sefer":8,"yog":"Az",       "fac":["Asansor","Otopark"],"aktarma":[]},
    {"id":"m3s2","isim":"Bagcilar M.",     "hat":"M3","renk":"#1e8449","x":135,"y":355,"lat":41.044,"lon":28.820,"km":3.5, "sefer":4,"yog":"Orta",     "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m3s3","isim":"Gungoren",        "hat":"M3","renk":"#1e8449","x":150,"y":445,"lat":41.028,"lon":28.843,"km":6.8, "sefer":6,"yog":"Orta",     "fac":["Asansor","Engelli","Bufe"],"aktarma":[]},
    {"id":"m3s4","isim":"Bagcilar",        "hat":"M3","renk":"#1e8449","x":170,"y":535,"lat":41.009,"lon":28.836,"km":9.2, "sefer":9,"yog":"Az",       "fac":["Asansor"],"aktarma":[]},
    {"id":"m4s1","isim":"Kadikoy",         "hat":"M4","renk":"#d35400","x":455,"y":350,"lat":40.990,"lon":29.028,"km":0,   "sefer":3,"yog":"Cok Yogun","fac":["Asansor","Engelli","AVM","Bufe","Danisma"],"aktarma":[]},
    {"id":"m4s2","isim":"Ayrilik Cesmesi", "hat":"M4","renk":"#d35400","x":480,"y":415,"lat":41.000,"lon":29.055,"km":2.1, "sefer":5,"yog":"Yogun",    "fac":["Asansor","Engelli","Bufe"],"aktarma":["M5"]},
    {"id":"m4s3","isim":"Unalan",          "hat":"M4","renk":"#d35400","x":492,"y":478,"lat":41.008,"lon":29.070,"km":4.5, "sefer":7,"yog":"Orta",     "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m4s4","isim":"Goztepe",         "hat":"M4","renk":"#d35400","x":505,"y":548,"lat":41.014,"lon":29.087,"km":7.0, "sefer":4,"yog":"Orta",     "fac":["Asansor","Engelli","Otopark"],"aktarma":[]},
    {"id":"m5s1","isim":"Uskudar",         "hat":"M5","renk":"#7d3c98","x":523,"y":315,"lat":41.023,"lon":29.015,"km":0,   "sefer":5,"yog":"Yogun",    "fac":["Asansor","Engelli","Bufe","Danisma"],"aktarma":[]},
    {"id":"m5s2","isim":"Fistikhagaci",    "hat":"M5","renk":"#7d3c98","x":558,"y":378,"lat":41.012,"lon":29.042,"km":2.4, "sefer":8,"yog":"Az",       "fac":["Asansor","Engelli"],"aktarma":[]},
    {"id":"m5s3","isim":"Baglarbasi",      "hat":"M5","renk":"#7d3c98","x":578,"y":450,"lat":41.003,"lon":29.054,"km":4.8, "sefer":6,"yog":"Orta",     "fac":["Asansor","Engelli","Bufe"],"aktarma":[]},
    {"id":"m5s4","isim":"Altunizade",      "hat":"M5","renk":"#7d3c98","x":590,"y":525,"lat":40.994,"lon":29.065,"km":7.3, "sefer":4,"yog":"Orta",     "fac":["Asansor","Engelli","Otopark"],"aktarma":[]},
]

HAT_CIZGILERI = {
    "M1":{"renk":"#c0392b","pts":[(60,510),(120,510),(185,470),(250,430),(310,390),(365,368)]},
    "M2":{"renk":"#2471a3","pts":[(80,55),(80,130),(130,185),(210,235),(290,272),(365,292),(425,292),(488,272),(558,232),(625,192),(685,162)]},
    "M3":{"renk":"#1e8449","pts":[(130,185),(130,268),(135,355),(150,445),(170,535)]},
    "M4":{"renk":"#d35400","pts":[(425,292),(455,350),(480,415),(492,478),(505,548)]},
    "M5":{"renk":"#7d3c98","pts":[(488,272),(523,315),(558,378),(578,450),(590,525)]},
}
...
def _bg_gorsel_yukle(genislik=920, yukseklik=590):
    """
    Arka plan gorselini dosyadan yukler ve isler.
    Desteklenen formatlar: JPEG, JPG, PNG, BMP, WEBP (PIL destekledigi her format).
    Gorsel bulunamazsa koyu degrade dondurur.
    """
    yol = gorsel_yolu_bul()

    if yol:
        try:
            img = Image.open(yol).convert("RGB").resize(
                (genislik, yukseklik), Image.LANCZOS
            )
            print(f"[Metro] Gorsel yuklendi: {os.path.basename(yol)}")
        except Exception as e:
            print(f"[Metro] Gorsel yuklenemedi ({yol}): {e}")
            img = _fallback_bg(genislik, yukseklik)
    else:
        print("[Metro] Gorsel bulunamadi, varsayilan arka plan kullaniliyor.")
        print(f"[Metro] Aranan klasor: {SCRIPT_DIR}")
        print(f"[Metro] Aranan isimler: {', '.join(BG_IMAGE_CANDIDATES)}")
        img = _fallback_bg(genislik, yukseklik)
    ...
    # Sag taraf gecis (form paneli icin)
    mask = Image.new("RGBA", (genislik, yukseklik), (0, 0, 0, 0))
    draw = ImageDraw.Draw(mask)
    for i in range(420):
        a = int(210 * (i / 420))
        draw.rectangle([genislik-420+i, 0, genislik-420+i+1, yukseklik],
                       fill=(5, 8, 22, a))
...
def _fallback_bg(genislik, yukseklik):
    """Gorsel yoksa koyu mavi degrade olustur."""
...
class LoginEkrani(tk.Toplevel):
    ...
    def _olustur(self):
        ...
        tk.Frame(panel, bg=GOLD_D, height=1).pack(fill="x", padx=28, pady=16)

        # Sunucu durum gostergesi
        self.srv_frame = tk.Frame(panel, bg="#0c1022")
        self.srv_frame.pack(pady=(0,8))
        self.srv_dot = tk.Canvas(self.srv_frame, width=10, height=10,
            bg="#0c1022", highlightthickness=0)
        ...
        tk.Label(frm, text="KULLANICI ADI", bg="#0c1022",
        ...
        tk.Label(frm, text="SIFRE", bg="#0c1022",
            fg=GOLD_D, font=("Helvetica",8)).pack(anchor="w")
        ...
        self.hata_lbl = tk.Label(frm, text="", bg="#0c1022",
        ...
        tk.Button(frm, text="SISTEME GIRIS YAP",
            ...
            command=self._giris).pack(fill="x", pady=6)

        tk.Label(panel, text="Demo: aasd / 1234",
            bg="#0c1022", fg=TEXT_M, font=("Helvetica",9)).pack(pady=(6,0))
    ...
    def _giris_async(self, u, p):
        ...
        if yanit.startswith("HATA"):
            if u in YEREL_KULLANICILAR and YEREL_KULLANICILAR[u]["sifre"] == p:
                ...
                self.after(0, lambda: self.hata_lbl.config(
                    text="Kullanici adi veya sifre hatali!"))
        else:
            if u in YEREL_KULLANICILAR:
                try:
                    if ":" in yanit:
                        bak = float(yanit.split(":")[-1].strip())
...
class MetroApp:
    ...
    def _navbar(self):
        ...
        nb.pack(fill="x"); nb.pack_propagate(False)
